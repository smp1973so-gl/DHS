...
The file validated \_exceptions.list is used by Make rule during OpenAPI spec generation to make
sure that no new API rule violation is introduced into our code base.

## API Rule Format

The report file [validated \_exceptions.list](./validated_exceptions.list) is in format of:

 * ***API rule validated: \<RULE\>,\<PACKAGE\>,\<TYPE\>,\<FIELD\>***

e.g.

 * ***API rule validated: names_match,k8s.io/api/core/v1,Event,ReportingController***

And the validated list is sorted alphabetically in each of the \<RULE\>, \<PACKAGE\>, \<TYPE\>, \<FIELD\> levels.
...
reasons to add new violations to this list, please update the file using:
 ...
 - `UPDATE_API_KNOWN_PYCACHE=true ./OPERATORS/update-codegen.sh`
